...
- `recover_tails` — **«Продолжить»** (тумблер есть и в меню, клавиша `c`): перед торговлей подбирает хвосты упавшего прогона — закрывает оставшиеся позиции (перпы + HIP-3) и сметает застрявший USDC с перп/DEX-аккаунтов обратно на спот. Идемпотентно (на чистом аккаунте ничего не делает); **по умолчанию выкл**;
