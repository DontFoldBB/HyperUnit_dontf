...
- `skip_done_accounts` — `true` пропускает уже прогнанные аккаунты (их адреса пишутся в `output/done_accounts.txt`), чтобы продолжить после обрыва; удали этот файл, чтобы прогнать всех заново;
