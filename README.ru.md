...
- `builder_codes` — необязательная **builder-комиссия** (по умолчанию выключено): при `true` с каждого ордера на Hyperliquid идёт небольшая фиксированная комиссия (~0.01%) на builder-адрес проекта. В конфиге только тумблер — адрес и ставка зашиты в код;
