...
- `skip_done_accounts` — `true` пропускает аккаунты, где **все стадии прошли** (пишутся в `output/done_accounts.txt`), чтобы продолжить после обрыва. Аккаунты с упавшей стадией идут в `output/failed_accounts.txt` и повторяются на следующем запуске. Удали `done_accounts.txt`, чтобы пройти всех заново;
