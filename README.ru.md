...
- `builder_codes` / `builder_address` / `builder_fee` — необязательная **builder-комиссия**: при `builder_codes: true` с каждого ордера на Hyperliquid идёт небольшая комиссия (`builder_fee`, напр. `0.01%`; макс 0.1% на перпах) на `builder_address`. **По умолчанию выключено.** На builder-адресе нужно ≥100 USDC в перпах (режим standard), иначе комиссия не начислится;
