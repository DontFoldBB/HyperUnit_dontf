...
- `skip_done_accounts` — `true` skips accounts already processed (their addresses are logged to `output/done_accounts.txt`) so you can resume after an interruption; delete that file to run everyone from scratch;
